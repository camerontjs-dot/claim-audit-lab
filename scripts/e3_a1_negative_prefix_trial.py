"""Test the preregistered ``neg`` extension to the E3 A1 parser guard.

The closed secondary-predicate trial remains immutable.  This module reuses its
recorded-trace replay machinery with a new, explicitly versioned candidate and
writes evidence only to the follow-up craft-trial folder.
"""
...
try:
    from scripts import e3_a1_secondary_predicate_trial as prior
    from scripts.simple_logic_gold_structured_lane import _write_absent_or_identical
except ModuleNotFoundError as exc:
    if exc.name != "scripts":
        raise
    import e3_a1_secondary_predicate_trial as prior  # type: ignore[no-redef]
...
_ALLOWED_IMPERATIVE_PREFIX_DEPS = frozenset({"intj", "advmod", "neg"})
...
def candidate_sentence_type(claim: str) -> SentenceType:
    """Apply the fixed ``neg`` prefix set plus the secondary-subject guard."""
...
def canary_report() -> dict[str, Any]:
    """Return the complete fixed 13-canary receipt."""
...
def build_trial_payload(project_root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    """Run the new candidate through the immutable prior replay machinery."""
    prior_dir = project_root / "outputs/2026-07-15-e3-a1-secondary-predicate-guard"
    ...
    prior_summary = json.loads((prior_dir / "trial-summary.json").read_text(encoding="utf-8"))
    ...
    summary = {
        **replay_summary,
        "schema_version": "e3-a1-negative-prefix-trial-v0.1",
...
def main() -> None:
    ...
    output_dir = project_root / "outputs/2026-07-15-e3-a1-negative-prefix-guard"
