...
| CAL-REQ-005 | Numeric claims must be matched against supplied evidence when numbers agree. | validation cases | `evidence_matching.py` | test | `tests/test_evidence_matching.py` | A fixture claim with the same number as an evidence excerpt receives a candidate evidence link. | Core traceability promise is weak. | planned |
...
| CAL-REQ-024 | Multiple candidate evidence sources must preserve reliability and support differences. | validation cases | `evidence_matching.py`, `rules.py`, `report.py` | test, analysis | `tests/test_evidence_matching.py`, `tests/test_rules.py`, `tests/test_report.py` | Candidate evidence links retain source IDs, excerpt IDs, scores, reliability labels, and warnings when support quality differs. | The report hides uncertainty or treats weak and strong sources as equivalent. | planned |
...
| Product README paragraph | capability, scope, comparative, prediction | `partially_supported`, `overstated`, freshness warning | test output, limitation note, dated source |
...
| Neutral/adverse scaffold cases | numeric, causal, interpretive, scope | no-change or worse-output outcomes | matched draft/evidence pairs where the scaffold does not help |
| False caution case | supported, unsupported, and omitted claims | false-caution warning or human-review flag | evidence that supports a conclusion the final answer refuses to state |
| Paired draft/final answer | repeated, removed, downgraded, added claims | movement counts plus limitations | draft/final pair with same evidence bundle |
...
- Research-use runs record version/config/evidence metadata before aggregate metrics are trusted.
- Independent validation fixtures include neutral, adverse, and false-caution cases.
- Human-review calibration is documented before using automated counts as study outcomes.
...
- Do not change research-facing rules after reviewing experiment outcomes unless the rerun is labeled exploratory or the tool version is frozen again.
