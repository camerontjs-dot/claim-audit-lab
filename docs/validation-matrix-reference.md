...
| CAL-REQ-041 | CAL must pin and verify the locked C-B contract vocabulary locally. | Apparatus Contracts v1.0.0 | `schema/`, `contracts/bundle_loader.py` | inspection, test | `schema/.contract-version`; `schema/vocabulary.yaml`; `tests/test_cb_bundle_loader.py`; workspace `scripts/audit_workspace.py` | CAL pins `1.0.0`, the local vocabulary copy matches canonical byte-for-byte, and optional bundled vocabulary drift fails closed. | Contract drift could silently change audit labels or schema expectations. | verified |
...
| CAL-REQ-046 | A synthetic C-A -> Evidence Bundler C-B -> CAL audited-output round trip must be documented and reproducible. | Unit 7 handoff gate | Evidence Bundler fixture, CAL CLI, validation docs | demonstration, inspection | `docs/verification.md`; Evidence Bundler `verify-intake` and `build-fixture-bundle`; CAL `audit-bundle`; `python3 scripts/audit_workspace.py` | The checked-in C-A fixture verifies, Evidence Bundler emits a C-B fixture bundle, CAL writes a reloadable audited C-B copy with a populated verdict, and the command sequence is documented with synthetic-fixture limits. | The apparatus could look connected on paper while lacking a reproducible engineering handoff. | verified |
