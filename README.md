...
| | |
|---|---|
| Distribution | `0.6.0` promotion candidate; adds the bounded `claim-audit-v1` Contract B 1.2 → CAL native verdict surface. Latest public release remains `0.5.0`. |
...
| Release tag | Latest public release: `v0.5.0`. The `0.6.0` promotion candidate is intentionally **untagged** until local pipeline acceptance and release authorization. **Not** `v1.0.0`. |
...
Not yet published to PyPI. The latest public release is `0.5.0`. This promotion branch carries
an unreleased `0.6.0` candidate for local CAL Pipeline runs; it adds the dedicated
`claim-audit-v1` Contract B 1.2 → native CAL verdict surface without replacing the historical
`claim-audit` command. Do not treat `0.6.0` as an official release until an immutable tag and
release record are created.
