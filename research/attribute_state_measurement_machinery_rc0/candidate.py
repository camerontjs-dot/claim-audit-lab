"""Competing proposal instruments for Attribute State Measurement Machinery RC0."""
...
def _hard_hazard(text: str) -> str | None:
    ...
    if low.startswith("other batch "):
        return "entity mismatch"
    if low.startswith("document status"):
...
def direct_declared_state_grammar(text: str) -> Observation:
    """Bounded declared-attribute grammar with explicit domain identity."""
    ...
    if match is not None:
        ...
        if value not in _KNOWN_VALUES[key]:
            return Observation.unresolved("value outside bounded direct vocabulary")
        return _claim(key[0], key[1], value)

    if " is " in f" {compact.casefold()} " or ":" in compact:
        return Observation.unresolved("state-like surface outside bounded grammar")
...
def broad_copular_state(text: str) -> Observation:
    """Broad copular/state extractor used to expose family-neighbor overreach."""
    ...
    diagnostic_patterns: tuple[
        ...
        (
            r"The batch's status is released",
            ("batch", "status", "released"),
        ),
        (
            r"Batch status:\s*released",
            ("batch", "status", "released"),
        ),
        (
            r"The device is in standby mode",
            ("device", "mode", "standby"),
        ),
        (
    ...
    for pattern, (entity, attribute, value) in diagnostic_patterns:
    ...
    # Broad copular fallback intentionally ignores several RC0 boundaries so
...
def conservative_state_hybrid(text: str) -> Observation:
    """Use broad extraction only for preregistered state-safe surfaces."""
