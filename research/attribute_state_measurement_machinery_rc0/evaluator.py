"""Frozen evaluator for CAL Attribute State Measurement Machinery RC0."""
...
from .cohort import Bucket, CASES, CASES_BY_ID, METAMORPHIC_PAIRS, Observation, StateAtom, Status
...
def weak_any_copula(text: str) -> Observation:
    if " is " in f" {text.casefold()} ":
        return Observation.claimed(StateAtom("batch","status","batch_status","released",True))
    return Observation.unresolved("weak")


def weak_membership_as_state(text: str) -> Observation:
    if "alice is a reviewer" in text.casefold():
        return Observation.claimed(StateAtom("alice","role","roles","reviewer",True))
    return Observation.unresolved("weak")


def weak_change_as_state(text: str) -> Observation:
    if "became held" in text.casefold():
        return Observation.claimed(StateAtom("batch","status","batch_status","held",True))
    return Observation.unresolved("weak")


def weak_reporting_erasure(text: str) -> Observation:
    if "batch status is released" in text.casefold():
        return Observation.claimed(StateAtom("batch","status","batch_status","released",True))
    return Observation.unresolved("weak")


def weak_nonfunctional_singleton(text: str) -> Observation:
    if "record tag is critical and reviewed" in text.casefold():
        return Observation.claimed(StateAtom("record","tag","labels","critical",False))
    return Observation.unresolved("weak")
