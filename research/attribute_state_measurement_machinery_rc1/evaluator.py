"""Frozen evaluator for Attribute State Measurement Machinery RC1."""
...
def weak_ignore_entity(text:str)->Observation:
    if "other batch status is released" in text.casefold():
        return Observation.claimed(StateAtom("batch","status","batch_status","released",True))
    return Observation.unresolved("weak")


def weak_ignore_domain(text:str)->Observation:
    if "document status is released" in text.casefold():
        return Observation.claimed(StateAtom("document","status","batch_status","released",True))
    return Observation.unresolved("weak")
