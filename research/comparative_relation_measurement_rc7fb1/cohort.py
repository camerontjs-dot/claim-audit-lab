...
COHORT_FREEZE_EXPECTED="rc7fb1-heldout-v1-semantics-first-20260831-r1"
CASES=[]
...
for i in range(6):
    l,r=ents(i); n=20+i; d=3+i; pid=f"delta-dir-{i}"
...
for i in range(4):
    l,r=ents(i+4); add(f"EQ{i:02d}","equality",f"{l} produced a total equal to {r}.",atom(l,"EQUAL_TO",r))
for i in range(4):
    l,r=ents(i+5); add(f"TW{i:02d}","multiplier",f"{l} produced twice as many units as {r}.",atom(l,"MULTIPLE_OF",r))
for i in range(4):
...
NEG=[
 "The higher folder contains archived notes.","The greater-than symbol appears in the manual.",
 "The lower shelf contains twelve files.","The smaller label is blue.","The exceeded flag is archived.",
 "The trailed column is hidden.","Sector A processed 12 units and Sector B processed 9 units.",
 "Sector A and Sector B are listed in the comparison table.","Sector A surpassed Sector B by four units.",
 "Sector A lagged behind Sector B.","Sector A was roughly comparable to Sector B.",
 "More documentation is available in Sector A.",
]
for i,text in enumerate(NEG): add(f"N{i:02d}","negative_or_unsupported",text,None)

# R1 changes only bookkeeping identity/count. The 68 case definitions above are
# byte-for-byte semantically identical to the failed first cohort freeze.
