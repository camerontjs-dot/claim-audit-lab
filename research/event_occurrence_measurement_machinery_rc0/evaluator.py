"""Frozen evaluator for Event Occurrence Measurement Machinery RC0."""
...
def weak_event_words(text: str) -> Observation:
    low = text.casefold()
    if "qa" in low and "approv" in low and "batch" in low:
        return Observation.claimed(EventAtom("qa", "approve", "batch", Polarity.OCCURRED))
    return Observation.unresolved("weak")


def weak_negation_erasure(text: str) -> Observation:
    return weak_event_words(text)
...
def weak_order_implies_occurrence(text: str) -> Observation:
    return weak_event_words(text)


def weak_deontic_implies_occurrence(text: str) -> Observation:
    return weak_event_words(text)
