"""Frozen evaluator for Typed Binary Relation Measurement Machinery RC0."""
...
def weak_keyword_relation(text:str)->Observation:
    low=text.casefold()
    if "own" in low and "a" in low and "b" in low:
...
def weak_all_spatial_transitive(text:str)->Observation:
    if "north" in text.casefold():
        return Observation.claimed(RelationAtom("a","NORTH_OF","c"))
    return Observation.unresolved("weak")


def weak_unknown_predicate(text:str)->Observation:
...
def weak_reporting_erasure(text:str)->Observation:
    return weak_keyword_relation(text)
