...
def _context(text: str, family: Any) -> Any:
    ...
    passage = AdmittedPassage.create("p1", "unseen-pressure-rc2-source", text)
    world = EvidenceWorld.create(
        ...
        (passage,),
        {
            "search_scope": {"corpus": "semantic-family-unseen-pressure-rc2"},
...
def _no(stage: str, detail: str) -> dict[str, str]:
    return {"status": "NOT_WARRANTED", "stage": stage, "detail": detail}


def _yes() -> dict[str, str]:
...
def probe_deontic(text: str) -> dict[str, str]:
    from research.deontic_gate1b_authority_rc0.candidate import complete_and_warrant_deontic
    ...
    if measured.status is not Status.CLAIMED or measured.norm is None:
        return _no("measurement", measured.status.value)
    try:
...
def probe_population(text: str) -> dict[str, str]:
    ...
    from research.population_measurement_machinery_rc0.candidate import conservative_hybrid
...
def probe_scalar(text: str) -> dict[str, str]:
    from research.scalar_gate1b_authority_rc0.candidate import complete_and_warrant_scalar
...
def probe_causal(text: str) -> dict[str, str]:
    ...
    from research.causal_measurement_machinery_rc0.candidate import conservative_assertion_hybrid
...
def main() -> None:
    ...
    for case in cases:
        ...
        rows.append({"case_id": case["case_id"], "family": family, **observed})
