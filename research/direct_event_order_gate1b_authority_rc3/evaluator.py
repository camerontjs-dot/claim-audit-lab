"""Frozen evaluator for direct-event-order Gate-1B authority RC3."""
...
CLEAN: tuple[Case, ...] = (
    Case("E3W01", "Talia reviewed packet u before Ravi signed ledger c.", True),
    Case("E3W02", "Talia reviewed packet u after Ravi signed ledger c.", True),
    Case("E3W03", "Talia did not review packet u before Ravi signed ledger c.", True),
    Case("E3W04", "Talia reviewed packet u after Ravi did not sign ledger c.", True),
)

REFUSE: tuple[Case, ...] = (
    Case("E3R01", "Perhaps Talia reviewed packet u before Ravi signed ledger c.", False),
    Case("E3R02", "If Talia reviewed packet u before Ravi signed ledger c.", False),
    Case("E3R03", "Talia reviewed packet u not before Ravi signed ledger c.", False),
    Case("E3R04", "Talia reviewed packet u immediately before Ravi signed ledger c.", False),
    Case("E3R05", "Talia reviewed packet u shortly before Ravi signed ledger c.", False),
    Case(
        "E3R06",
        "Talia reviewed packet u before Ravi signed ledger c because QA requested it.",
        False,
    ),
    Case("E3R07", "Allegedly Talia reviewed packet u before Ravi signed ledger c.", False),
    Case("E3R08", "Reportedly Talia reviewed packet u before Ravi signed ledger c.", False),
    Case(
        "E3R09",
    ...
    Case("E3R11", "QA approved the batch before Ops released it.", False),
    Case("E3R12", "She reviewed packet u before Ravi signed ledger c.", False),
)
...
def _context(case: Case) -> AuditContext:
    ...
    world = EvidenceWorld.create(
        "1.2.0",
        f"event-rc3-{case.case_id.lower()}",
        f"sha256:{_hex('bundle:' + case.case_id)}",
        (passage,),
        {
            "search_scope": {"corpus": "event-order-authority-rc3"},
