"""Frozen evaluator for CAL Causal Measurement Machinery RC0."""
...
from .cohort import Bucket, CASES, CASES_BY_ID, METAMORPHIC_PAIRS, CausalAtom, Kind, Observation, Status
...
def weak_everything_causes(text: str) -> Observation:
    low = text.casefold()
    if "a" in low and "b" in low:
        return Observation.claimed(CausalAtom("a", "b", Kind.CAUSES))
    return Observation.unresolved("weak")


def weak_ignore_endpoints(text: str) -> Observation:
    if "causes" in text.casefold():
        return Observation.claimed(CausalAtom("a", "b", Kind.CAUSES))
    return Observation.unresolved("weak")
...
def weak_association_is_correlation(text: str) -> Observation:
    if "associated with" in text.casefold():
        return Observation.claimed(CausalAtom("a", "b", Kind.CORRELATES_WITH))
    return Observation.unresolved("weak")


def weak_risk_factor_contribution(text: str) -> Observation:
    if "risk factor" in text.casefold():
        return Observation.claimed(CausalAtom("a", "b", Kind.CONTRIBUTES_TO))
    return Observation.unresolved("weak")


def weak_composition_collapse(text: str) -> Observation:
    if "causes b and c" in text.casefold():
        return Observation.claimed(CausalAtom("a", "b", Kind.CAUSES))
    return Observation.unresolved("weak")
