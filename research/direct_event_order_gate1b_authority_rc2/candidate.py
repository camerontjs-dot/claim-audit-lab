"""Scope- and coreference-aware event-order source completion for Gate-1B RC2."""
...
def _parse_event(text: str) -> dict[str, str]:
    ...
    object_tokens = set(event["object"].split())
    if object_tokens & _PRONOUN_OBJECTS:
        raise AuthorityRefusal(
            "SOURCE_COMPLETION_FAILED",
            "event object contains unresolved coreference",
        )
    return event
...
def complete_and_warrant_event_order_rc2(
