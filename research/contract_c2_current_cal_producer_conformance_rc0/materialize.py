...
def public_terminal_state(result: AuditResult) -> tuple[str, str]:
    ...
    # Contract C publishes stable epistemic reason classes, not CAL's internal
    # stage-local failure telemetry. Any actual unresolved categorical relation
    # remains public because it changes the epistemic state and precedence.
    if any(
    ...
    # All other completed non-deciding outcomes remain native-localized in CAL
    # but are the same public Contract C state: no deciding relation.
...
def materialize_unsealed(
    ...
    *,
    ...
    if _HEX40.fullmatch(semantic_implementation_sha) is None:
        raise ValueError("semantic_implementation_sha must be 40 lowercase hex")
