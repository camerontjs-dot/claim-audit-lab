...
CANDIDATE_VERSION = "rc7fa1-stacked-scope-1"
...
DEONTIC_PATTERNS = (
    r"\bonly\b.{0,80}\bmay\b", r"\bpermitted to\b", r"\ballowed to\b",
    r"\bauthorized to\b", r"\brequired to\b", r"\bmust\b", r"\bshall\b",
    ...
    for i, ch in enumerate(text):
        if ch == "“":
            ...
            if stack is not None:
                ranges.append((stack, i + 1)); stack = None
        elif ch == '"':
            if stack is None:
                stack = i
            else:
...
def _evidential_prefix(lower: str, start: int) -> tuple[int, str] | None:
    prefix = lower[:start]
    # Only an immediately framing adverb at the beginning of the current sentence/
    # clause qualifies. This prevents an unrelated earlier domain-word mention from
    # controlling the anchored observation.
    for word in EVIDENTIAL_ADVERBS:
        m = re.search(rf"(?:^|[.!?]\s+)({word})\b\s*(?:[,;:]|[-–—])?\s*$", prefix)
        if m:
            return m.start(1), word
    return None
...
def classify(raw_source: str, observation: dict) -> dict:
    ...
    if ev:
        detections.append((ev[0], seq, "UNRESOLVED_EVIDENTIAL", f"evidential:{ev[1]}")); seq += 1

    # Other explicitly unresolved framing adjacent to the observation.
    pre = lower[:start]
    ...
    if "according to " in pre[max(0, start - 120):]:
        p = pre.rfind("according to ")
        detections.append((p, seq, "UNRESOLVED_EVIDENTIAL", "according_to")); seq += 1
    if "it is disputed whether " in pre[max(0, start - 120):]:
        p = pre.rfind("it is disputed whether ")
        detections.append((p, seq, "UNRESOLVED_EVIDENTIAL", "disputed_whether")); seq += 1

    # Conditional jurisdiction is outermost when the source starts with a supported
    # conditional wrapper. Its clause position is determined against the first comma.
    stripped_offset = len(raw_source) - len(raw_source.lstrip())
    stripped = lower.lstrip()
    if any(stripped.startswith(p) for p in CONDITIONAL_PREFIXES):
        comma = raw_source.find(",")
        if comma < 0:
            detections.append((stripped_offset, seq, "UNRESOLVED_EVIDENTIAL", "conditional_boundary_unresolved")); seq += 1
        elif end <= comma + 1:
            detections.append((stripped_offset, seq, "CONDITIONAL_ANTECEDENT", "conditional_anchor_before_comma")); seq += 1
        elif start > comma:
            detections.append((stripped_offset, seq, "CONDITIONAL_CONSEQUENT", "conditional_anchor_after_comma")); seq += 1

    # Attribution may be nested inside a conditional or contain epistemic scope.
    for q0, q1 in _quote_ranges(raw_source):
        if start >= q0 and end <= q1:
            detections.append((q0, seq, "ATTRIBUTED", "anchor_inside_quote")); seq += 1
            break
    ...
    if that_pos >= 0:
        ...
        for v in REPORTING_VERBS:
            for m in re.finditer(rf"\b{re.escape(v)}\b", prefix):
                verb_hits.append((m.start(), v))
        if verb_hits:
            p, v = max(verb_hits)
            detections.append((p, seq, "ATTRIBUTED", f"reporting_verb:{v}")); seq += 1

    # Scope signals are retained rather than short-circuited. Only signals whose
    # textual occurrence begins no later than the anchored observation are eligible
    # to count as enclosing scope in this bounded candidate.
    ...
    for pat in QUANTIFIER_PATTERNS:
        ...
        if m and m.start() <= start:
            detections.append((m.start(), seq, "QUANTIFIED", f"quantifier:{pat}")); seq += 1
            break

    # Deduplicate same path type at same position while retaining distinct nested
    # types. Ordered by source position, then detection order.
    ...
    if path:
        legacy = path[0]
        if legacy == "UNRESOLVED_EVIDENTIAL":
            legacy = "UNRESOLVED"
...
def _result(scope_status: str, scope_path: list[str], basis: list[str]) -> dict:
    for item in scope_path:
        assert item in PATH_TYPES
    eligible = not scope_path and scope_status in {"ASSERTED", "ASSERTED_NEGATIVE"}
    return {
