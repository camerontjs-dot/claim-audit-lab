...
def main() -> None:
    ...
    for family in subjects:
        ...
        all_rows.extend(json.loads(completed.stdout))
    ...
    for case in cases:
        ...
        warrants = sorted(row["family"] for row in rows if row["status"] == "WARRANTED")
        ...
        if errors or warrants != allowed:
            failures.append({**item, "probe_errors": errors})
