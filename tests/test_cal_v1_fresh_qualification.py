...
_RECORD_DIR = _ROOT / "research" / "cal_v1_qualification_20260911"
...
_FROZEN_SHA = "dca2ec7f2c117cc0b4d6c543ab5049cac7994298"
...
def _run_case(case: dict[str, Any]) -> dict[str, Any]:
    ...
    if kind == "same_id_proposition_substitution":
        ...
        authority = complete_and_warrant(context, receipt, passage_id)
        ...
        try:
            relation = derive_relation(substituted, authority)
        except Exception as exc:  # evaluator records exact fail-closed surface
            return {
                "outcome": "REJECTED",
    ...
    if kind == "passage_substitution_stale_hash":
        ...
        packet: dict[str, Any] = {
            ...
            "evidence_world": {
                ...
                "bundle_id": "q10-stale-hash",
                "bundle_hash": _tagged("q10-stale-hash"),
    ...
    if kind in {"stale_authority_world", "tampered_authority_identity"}:
        source = _context(case, bundle_id=str(case["source_bundle"]))
        target = _context(case, bundle_id=str(case["target_bundle"]))
        passage_id = source.evidence_world.admitted_passages[0].passage_id
        receipt = measure_strict_comparison(source, passage_id)
        authority = complete_and_warrant(source, receipt, passage_id)
        candidate_authority = authority
        if kind == "tampered_authority_identity":
            tampered_atom = replace(
                authority.atom,
        ...
        try:
            relation = derive_relation(target, candidate_authority)
        except Exception as exc:  # evaluator records exact fail-closed surface
            return {
                "outcome": "REJECTED",
                "exception": type(exc).__name__,
        ...
        return {
            "outcome": "ACCEPTED",
            "categorical_relation": relation.categorical_relation.value,
            "authority_id": candidate_authority.authority_id,
            "atom_id": candidate_authority.atom.atom_id,
        }
...
def _compare(case_id: str, observed: dict[str, Any]) -> list[str]:
    ...
    if trace_failure is not None and trace_failure not in observed.get(
        ...
        failures.append(
            f"trace failure {trace_failure!r} absent from {observed.get('trace_failure_codes')!r}"
        )
...
def test_cal_v1_fresh_qualification_cohort() -> None:
    ...
    summary = {
        "schema": "cal-v1-fresh-qualification-observation-v1",
