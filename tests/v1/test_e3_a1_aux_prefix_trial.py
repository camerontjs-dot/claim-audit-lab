"""Focused no-inference checks for the preregistered A1 ``aux`` trial."""
...
from scripts.e3_a1_aux_prefix_trial import (
    build_trial_payload,
    canary_report,
...
def test_auxiliary_imperatives_and_declarative_controls() -> None:
    assert candidate_sentence_type("Do not release the batch.") == "imperative"
    assert candidate_sentence_type("Do release the batch.") == "imperative"
    assert candidate_sentence_type("Please do not release the batch.") == "imperative"
    assert candidate_sentence_type("The system does not release the batch.") == "declarative"
    assert candidate_sentence_type("Can users release the batch?") == "question"


def test_complete_canary_set_is_green() -> None:
    report = canary_report()
    assert report["denominator"] == 17
    assert report["matching"] == 17
    assert report["failures"] == []


def test_replays_are_identical_to_prior_improvement_and_candidate_is_keep() -> None:
    ...
    assert summary["recommended_verdict"] == "keep"
...
def test_trial_writes_repeatable_absent_or_identical_artifacts(tmp_path: Path) -> None:
    ...
    assert json.loads(summary_bytes)["all_criteria_pass"] is True
